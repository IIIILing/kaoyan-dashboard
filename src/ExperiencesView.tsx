        ...
        <div className="milestone-list">
          {subject.milestones.map((entry) => (
            <article className="milestone-editor" key={entry.id}>
              ...
              <label className="milestone-phase"><span>对应进度阶段</span>{subjectPhases.length ? <select value={entry.phaseId ?? ""} onChange={(event) => onUpdateMilestone(entry.id, { phaseId: event.target.value || undefined })}><option value="">不参与快线</option>{subjectPhases.map((phase) => <option value={phase.id} key={phase.id}>{phase.name}</option>)}</select> : <input placeholder="phaseId（可选）" value={entry.phaseId ?? ""} onChange={(event) => onUpdateMilestone(entry.id, { phaseId: event.target.value || undefined })} />}</label>
              <label className="milestone-date"><span>开始 MM-DD</span><input placeholder="07-15" value={entry.startMonthDay} onChange={(event) => onUpdateMilestone(entry.id, { startMonthDay: event.target.value })} /></label>
              <label className="milestone-date"><span>结束 MM-DD</span><input placeholder="09-15" value={entry.endMonthDay} onChange={(event) => onUpdateMilestone(entry.id, { endMonthDay: event.target.value })} /></label>
              <label className="milestone-meta"><span>日期精度</span><select value={entry.datePrecision} onChange={(event) => onUpdateMilestone(entry.id, { datePrecision: event.target.value as DatePrecision })}><option value="explicit">原文明确</option><option value="approximate">原文约数</option><option value="inferred">据上下文推断</option></select></label>
              <label className="milestone-meta"><span>来源 PDF 页</span><input value={pageText(entry.sourcePages)} onChange={(event) => onUpdateMilestone(entry.id, { sourcePages: pageList(event.target.value) })} /></label>
              <label className="milestone-workload"><span>工作量</span><input value={entry.workload} onChange={(event) => onUpdateMilestone(entry.id, { workload: event.target.value })} /></label>
              <label className="milestone-detail"><span>阶段详情</span><textarea rows={3} value={entry.detail} onChange={(event) => onUpdateMilestone(entry.id, { detail: event.target.value })} /></label>
